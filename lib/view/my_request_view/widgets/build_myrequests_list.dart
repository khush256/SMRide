import 'package:flutter/material.dart';
import 'package:provider/provider.dart';
import 'package:smride_app/view/my_request_view/widgets/my_request_card.dart';
import 'package:smride_app/model/fetch_request_model.dart';
import 'package:smride_app/view_model/home_view_model.dart';

Widget buildMyRequestList(
  BuildContext context,
  // List<FetchRequestModel> requests
) {
  List<FetchRequestModel> myRequests =
      context.read<HomeViewModel>().myRequests.data;
  return RefreshIndicator(
    color: Colors.black,
    onRefresh: context.read<HomeViewModel>().fetchMyRequests,
    child: myRequests.isNotEmpty
        ? ListView.builder(
            itemCount: context.read<HomeViewModel>().myRequests.data.length,
            itemBuilder: (BuildContext context, int index) {
              return myRequestCard(context, myRequests[index]);
            },
          )
        : ListView(
            physics:
                const AlwaysScrollableScrollPhysics(), // ensures scroll even if no items
            children: [
              SizedBox(
                height: MediaQuery.of(context).size.height *
                    0.8, // to center the text vertically
                child: Center(
                  child: Text(
                    "No Requests",
                    style: TextStyle(
                      fontSize: 20,
                      color: Colors.grey,
                    ),
                  ),
                ),
              ),
            ],
          ),
  );
}
